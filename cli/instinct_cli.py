...
def _print_data_stats():
    ...
    episodes = DATA_DIR / "episodes.json"
    if episodes.exists():
        try:
            d = json.loads(episodes.read_text())
            print(f"  Episodes: {d.get('total_turns', 0)} turns, "
                  f"{d.get('total_sessions', 0)} sessions")
        except Exception:
            pass
    ...
    if analyzed.exists():
        try:
            d = json.loads(analyzed.read_text())
...
def cmd_materialize(args):
    ...
    # Generate delegation prompt templates
    ...
    if delegation:
        ...
        for inst in delegation:
            ...
            # Try to extract agent type from content or id
            agent = 'general'
    ...
    if args.dry_run:
        print("\n[DRY RUN] Would have written the above files.")
        # Could read back and display, but already printed paths
    else:
